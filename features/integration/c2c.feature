...
Feature: Contract to Contract Interaction
  Scenario:
    ...
    * I add a method call with the transient account, the 0th app and previous apps foreign, suggested params, on complete "noop", current transaction signer, current method arguments.
    * I build the transaction group with the composer. If there is an error it is "".
    ...
    And Ze atomic result for randomInt() at index 0 proves correct for input "AAAAAAAAACo="
    And I can retrieve all inner transactions that were called from the atomic transaction with call graph "{appl->{}}".
    ...
    And a new AtomicTransactionComposer
    * I create the Method object from method signature "randElement(string,application)(byte,byte[17])"
    * I create a new method arguments array.
    * I append the encoded arguments "AAVoZWxsbw==,ctxAppIdx:0" to the method arguments array.
    * I add a method call with the transient account, the 1th app and previous apps foreign, suggested params, on complete "noop", current transaction signer, current method arguments.
    * I build the transaction group with the composer. If there is an error it is "".
    Then I gather signatures with the composer.
    And I execute the current transaction group with the composer.
    Then The composer should have a status of "COMMITTED".
    ...
    # spin() -> (result, witness1, witness2, witness3)
    And a new AtomicTransactionComposer
    * I create the Method object from method signature "spin()(byte[3],byte[17],byte[17],byte[17])"
    * I create a new method arguments array.
    * I add a method call with the transient account, the 2th app and previous apps foreign, suggested params, on complete "noop", current transaction signer, current method arguments.
    * I build the transaction group with the composer. If there is an error it is "".
# Then I gather signatures with the composer.
# And I execute the current transaction group with the composer.
# Then The composer should have a status of "COMMITTED".
