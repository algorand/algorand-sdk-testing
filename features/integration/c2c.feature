...
Feature: Contract to Contract Interaction
  ...
  Scenario: Playing at the Casino using Contract to Contract Invocations
    ...
    * I build an application transaction with the transient account, the current application with method args, suggested params, operation "create", approval-program "programs/test_itxn_appargs.teal", clear-program "programs/one.teal.tok", global-bytes 0, global-ints 1, local-bytes 0, local-ints 0, foreign-apps "", foreign-assets "", app-accounts "", extra-pages 0
    And I sign and submit the transaction, saving the txid. If there is an error it is "".
    And I wait for the transaction to be confirmed.
    Given I remember the new application ID.

# # application 1: RandomByte (really 2... but can we get to this point)
# And I create a new method arguments array.
